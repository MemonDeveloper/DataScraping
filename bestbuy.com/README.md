# BestBuy Data Extractor 🤖📊

This Python application provides a graphical user interface (GUI) to automate extracting product details from `bestbuy.com`. It's designed to read product links from an Excel file, use `undetected-chromedriver` to navigate to those pages, extract various product information, and then save all the collected data into a new Excel file.

-----

## Features ✨

  * **GUI-Driven**: An easy-to-use interface built with `tkinter`.
  * **Automated Navigation**: Uses `undetected-chromedriver` to bypass bot detection and navigate `bestbuy.com` product pages.
  * **Data Extraction**: Gathers comprehensive details from each product, including:
      ...
      * Product Link
  * **Excel Integration**:
      * Reads product links from a specified Excel sheet (`bestbuy.xlsx`).
...
1.  **Clone the repository** (or download the `main.py` file directly):
    ...
    ```bash
    git clone https://github.com/MemonDeveloper/bestbuy.com.git
    cd bestbuy.com
    ```
...
1.  **Prepare your input Excel file**:
      ...
      * Create an Excel file named `bestbuy.xlsx` in the same directory as the script.
      * Inside this Excel file, create a sheet named exactly `bestbuy List`.
      * In this sheet:
          * Column **A** should contain the **Search ID**.

    *Example `bestbuy.xlsx` structure:*
...
3.  **Interact with the GUI**:
      ...
      * A simple Tkinter window titled "Memon v.1.15" will appear.
      * Click the **"bestbuy"** button to start the process.
...
The script will create a new Excel file (e.g., `bestbuy - Result-MM-DD HH-MM-SS.xlsx`) in the same directory. This file will contain two sheets:

1.  **`bestbuy List`**: This is a copy of your original input sheet (Column A).
2.  **`Result`**: This new sheet will contain the extracted data, with the following columns:
      * **Product Link (original)**
      * **Status**
      * **Buyable**
      * **Discontinued**
      * **Retail Price**
...
## Important Notes & Disclaimer ⚠️
  ...
  * **Website Specificity**: This script is **highly specialized** for the current HTML structure and product page layout of `https://www.bestbuy.com/`. Any changes to the website's design (e.g., element IDs, class names, navigation paths) will likely break the script, requiring updates to the Selenium selectors.
  * **Ethical Use**: Please use this tool responsibly and in compliance with the terms of service of `bestbuy.com`. Unauthorized or excessive scraping can lead to your IP being blocked. The developers of this script are not responsible for any misuse.
  ...
  * **Performance**: Processing a large number of product links may take a considerable amount of time.
