...
## Features ✨
  ...
  * **Automated Navigation**: Uses `undetected-chromedriver` to bypass bot detection and navigate `lowes.com` product pages.
  * **Data Extraction**: Gathers comprehensive details from each product, including:
      ...
      * Product Link
  * **Excel Integration**:
      * Reads product links from a specified Excel sheet (`lowes.xlsx`).
...
1.  **Prepare your input Excel file**:
      ...
      * Create an Excel file named `lowes.xlsx` in the same directory as the script.
      * Inside this Excel file, create a sheet named exactly `lowes List`.
      * In this sheet:
          * Column **A** should contain the **Search ID**.

    *Example `lowes.xlsx` structure:*
...
3.  **Interact with the GUI**:
      ...
      * Click the **"lowes"** button to start the process.
...
The script will create a new Excel file (e.g., `lowes - Result-MM-DD HH-MM-SS.xlsx`) in the same directory. This file will contain two sheets:

1.  **`lowes List`**: This is a copy of your original input sheet (Column A).
...
## Important Notes & Disclaimer ⚠️
  ...
  * **Website Specificity**: This script is **highly specialized** for the current HTML structure and product page layout of `https://www.lowes.com/`. Any changes to the website's design (e.g., element IDs, class names, navigation paths) will likely break the script, requiring updates to the Selenium selectors.
  * **Ethical Use**: Please use this tool responsibly and in compliance with the terms of service of `lowes.com`. Unauthorized or excessive scraping can lead to your IP being blocked. The developers of this script are not responsible for any misuse.
